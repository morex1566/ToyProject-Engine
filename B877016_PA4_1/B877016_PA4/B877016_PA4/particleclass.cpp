...
bool ParticleSystemClass::InitializeParticleSystem()
{
	...
	{
		m_particleList[i].isActive = false;
	}
...
void ParticleSystemClass::EmitParticles(float frameTime)
{
	...
	if ((emitParticle == true) && (m_currentParticleCount < (m_maxParticles - 1)))
	{
		...
		while (!found)
		{
			if ((m_particleList[index].isActive == false) || (m_particleList[index].position.z < positionZ))
			{
				found = true;
			}
			else
		...
		while (i != index)
		{
			...
			m_particleList[i].color.x = m_particleList[j].color.x;
			m_particleList[i].color.y = m_particleList[j].color.y;
			m_particleList[i].color.z = m_particleList[j].color.z;
			m_particleList[i].velocity = m_particleList[j].velocity;
			m_particleList[i].isActive = m_particleList[j].isActive;
			i--;
			j--;
		}

		// Now insert it into the particle array in the correct depth order.
		m_particleList[index].position.x = positionX;
		m_particleList[index].position.y = positionY;
		m_particleList[index].position.z = positionZ;
		m_particleList[index].color.x = red;
		m_particleList[index].color.y = green;
		m_particleList[index].color.z = blue;
		m_particleList[index].velocity = velocity;
		m_particleList[index].isActive = true;
	}
...
void ParticleSystemClass::UpdateParticles(float frameTime)
{
	...
	{
		m_particleList[i].position.y = m_particleList[i].position.y - (m_particleList[i].velocity * frameTime * 0.001f);
	}
...
void ParticleSystemClass::KillParticles()
{
	...
	for (i = 0; i < m_maxParticles; i++)
	{
		if ((m_particleList[i].isActive == true) && (m_particleList[i].position.y < -3.0f))
		{
			m_particleList[i].isActive = false;
			m_currentParticleCount--;
			...
			for (j = i; j < m_maxParticles - 1; j++)
			{
				...
				m_particleList[j].color.x = m_particleList[j + 1].color.x;
				m_particleList[j].color.y = m_particleList[j + 1].color.y;
				m_particleList[j].color.z = m_particleList[j + 1].color.z;
				m_particleList[j].velocity = m_particleList[j + 1].velocity;
				m_particleList[j].isActive = m_particleList[j + 1].isActive;
			}
		}
	}
...
bool ParticleSystemClass::UpdateBuffers(ID3D11DeviceContext* deviceContext)
{
	...
	{
		// Bottom left.
		m_vertices[index].position = XMFLOAT3(m_particleList[i].position.x - m_particleSize, m_particleList[i].position.y - m_particleSize, m_particleList[i].position.z);
		m_vertices[index].texture = XMFLOAT2(0.0f, 1.0f);
		m_vertices[index].color = XMFLOAT4(m_particleList[i].color.x, m_particleList[i].color.y, m_particleList[i].color.z, 1.0f);
		index++;
